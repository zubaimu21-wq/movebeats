...
export default function App(){
 ...
 const [previewUrl,setPreviewUrl]=useState(null); const preview=useRef(null);
 ...
 function togglePreview(u){if(previewUrl===u){setPreviewUrl(null);return}setPreviewUrl(u)}
 function useTrack(t){setMusic(t.url);setMusicName(t.name);setMessage(`"${t.name}" set as timer music.`);setPreviewUrl(null)}
 ...
 <aside><section className="panel"><h2>Timer settings</h2><div className="grid"><label>Active seconds<input type="number" min="1" value={work} onChange={e=>{const v=+e.target.value;setWork(v);if(!running&&phase==='work')setLeft(v)}}/></label><label>Rest seconds<input type="number" min="0" value={rest} onChange={e=>setRest(+e.target.value)}/></label><label>Rounds<input type="number" min="1" max="99" value={rounds} onChange={e=>setRounds(+e.target.value)}/></label><label>End countdown<input type="number" min="1" max="60" value={endCount} onChange={e=>setEndCount(+e.target.value)}/></label></div><div className="lib"><h3>Music library</h3><div className="lib-list">{library.map(t=><div key={t.url} className={`lib-item${music===t.url?' active':''}`}><button className="icon" onClick={()=>togglePreview(t.url)} title="Preview">{previewUrl===t.url?<Pause/>:<Play/>}</button><span>{t.name}</span><button onClick={()=>useTrack(t)}>{music===t.url?'In use':'Use'}</button></div>)}</div>{previewUrl&&<audio key={previewUrl} src={previewUrl} autoPlay onEnded={()=>setPreviewUrl(null)}/>}</div><label className="upload"><Upload/> Browse your music<input type="file" accept="audio/*" onChange={pickAudio}/></label>{music&&<audio ref={audio} src={music} loop preload="metadata"/>}</section>
